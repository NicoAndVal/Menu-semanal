...
class Desayunos extends React.Component{
    ...
    render() {
        ...
        const showBreakfast = arrayDesayunos.map(desayuno =>{
            return(
                <div key={desayuno._id} className='calendario-comidas_plato'>
                    <h5> {desayuno.nombre} </h5>
                    <Link to={`/menu/${desayuno._id}`}><img className='calendario-comidas_imagen' src={require(`../../../public/uploads/${desayuno.img}`)}/></Link>
                    <h6 className ='parrafo' >{desayuno.time}</h6>
                </div>
            )
        })

        const randomLunch = showBreakfast.sort(()=>Math.random()-0.5)
        return(
